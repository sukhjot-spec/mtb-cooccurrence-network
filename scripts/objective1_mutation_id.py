#!/usr/bin/env python3
"""
Objective 1 - Identify genomic mutations associated with antimicrobial drug
resistance among African Mycobacterium tuberculosis isolates.

FORWARD-COMPATIBILITY NOTE (read this before touching Objectives 2-4):
This script is the ONE place mutation_id gets assigned. Every later step
(Objective 2's co-occurrence testing, Objective 3's network nodes,
Objective 4's per-lineage repeats) must read mutation_id from this script's
own output (core_variants_with_mutation_id.csv / mutation_node_table.csv)
rather than recomputing gene+change concatenation itself. mutation_id is
built once, here, as f"{gene}|{change}" (verified against the real data:
neither column contains a "|" character, so this is a safe, stable,
human-readable separator) - if any later script builds its own id from
gene+change with a different separator or formatting, joins across steps
will silently fail to match. Treat mutation_id as an immutable contract.

*** CRITICAL METHODOLOGICAL FINDING - READ BEFORE INTERPRETING obj1_fisher_
    results_core.csv ***
Testing core-set (Assoc w R) mutations against y_labels.csv's per-drug
_binary columns is CIRCULAR, not confirmatory, and this was confirmed
directly against the real data before this script was finalised: across
all 10,199 core mutation-drug rows, with ZERO exceptions, every sample
carrying a core mutation for drug D is marked resistant to D. TB-Profiler's
per-drug binary/categorical phenotype columns are themselves generated
from the same Assoc-w-R catalogue tier being tested against them - they
are a genotype-derived PREDICTION, not an independent phenotypic (culture
DST) measurement. Running Fisher's exact test between a core mutation and
its own drug's binary column therefore recovers the catalogue's own
resistance-calling rule for this cohort (near-infinite odds ratios,
near-zero p-values, for essentially every mutation) rather than providing
new evidence. This is expected and is not a bug in this script.

The genuinely meaningful confirmatory test runs on the SENSITIVITY set
("Uncertain significance" tier) instead: checked directly, only 64.9% of
sensitivity-set mutation-drug carriers are marked resistant to that drug
(35.1% are marked susceptible despite carrying the variant) - real,
non-circular variance, because the binary phenotype columns are NOT
derived from Uncertain-significance evidence. A significant, strong
association here is genuine cohort-specific evidence that could support
promoting a specific Uncertain-significance variant, which is exactly
the kind of finding "not just trusting catalog membership, actually
demonstrating the association in this cohort" (per the original project
plan) is asking for. Both tests are run and saved, clearly labelled and
separated, so this distinction is never lost in a later step: obj1_fisher_
results_core.csv is background/consistency-check output only,
obj1_fisher_results_sensitivity.csv is the real confirmatory analysis.

WHAT THIS SCRIPT DOES, IN ORDER:
  1. Loads core_set_with_metadata.csv (Phase A's output - 10,199 rows,
     confidence-confirmed resistance mutations only) and assigns mutation_id.
  2. Builds mutation_node_table.csv - ONE row per unique (gene, change),
     with all associated drugs listed as a single attribute (not one row
     per drug - this is the same node-identity decision already verified
     for the sibling-drug pairs, e.g. rifampicin/rifapentine share an
     identical carrier set, so a mutation must not be split into two nodes
     just because it confers resistance to two drugs). This table is the
     canonical node list Objective 3's network will be built from directly.
  3. Builds mutation_drug_frequency.csv - the same information in long
     format (one row per mutation-drug pair), for drug-specific reporting.
  4. Runs the confirmatory Fisher's exact test TWICE - once on the core
     set (background/consistency check only, see the circularity note
     above) and once on the sensitivity set (the real confirmatory
     analysis) - BH-FDR corrected separately within each, since they are
     two different families of hypotheses, not one combined test.

THE QC EXCLUSION, AND WHY IT'S HANDLED HERE, ONCE, EXPLICITLY:
Section 4 of the Phase A QC exploration established that 53 samples'
drtype/phenotype columns cannot be trusted as ground truth (36 are
QC-outlier samples with implausible total_variants_qc; 42 have a
drtype-implied resistance classification not supported by any
confidence-confirmed mutation - all 42 traced to Uncertain-significance or
unlabelled evidence, never to a confirmed mutation). This script excludes
those 53 samples from BOTH sides of every confirmatory Fisher's test
(neither their genotype nor their phenotype is used in that specific test)
- not just their phenotype label alone, because a 2x2 co-occurrence test
needs both axes to come from a population whose evidence is trusted. This
applies uniformly to both the core-set and sensitivity-set tests. Mutation
carriers remain in the node/frequency tables for both tiers regardless;
only the confirmatory PHENOTYPE tests exclude these 53 samples.

EDGE CASES CONFIRMED IN THE REAL DATA AND HANDLED EXPLICITLY (do not
remove these checks - all fire on the real, current dataset, not just in
theory):
  - rpoB p.Ser431Thr's only carrier (SRR33383939) is one of the 53 excluded
    samples, leaving zero carriers in the test population for that specific
    core-set mutation. Tests with zero carriers after exclusion are skipped
    with an explicit "insufficient_data" status, never silently run on a
    degenerate 2x2 table.
  - para-aminosalicylic_acid and cycloserine have zero core-set mutations
    at all, so no core-set (mutation, drug) test is ever generated for
    them. Expected, not a bug - logged explicitly.
  - Core-set odds ratios are mathematically infinite whenever the "mutation
    present but susceptible" cell is zero, which (per the circularity
    finding above) is EVERY core-set test. scipy.stats.fisher_exact still
    returns a valid p-value in this situation via the exact hypergeometric
    calculation; only the odds-ratio point estimate is undefined/infinite.
    This is left as inf rather than clipped to a large finite number, so
