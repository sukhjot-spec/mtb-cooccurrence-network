#!/usr/bin/env python3
"""
Objective 2 - Determine patterns of co-occurrence among drug-resistance-
associated mutations in the study isolates.

FORWARD-COMPATIBILITY NOTE:
This script reads mutation_id and the node list directly from Objective 1's
output (mutation_node_table.csv, core_variants_with_mutation_id.csv) rather
than recomputing anything. Objective 3 will read this script's
cooccurrence_significant_edges.csv directly as its network edge list - do

*** OBJECTIVE 4 REUSE: THE -lineage ARGUMENT ***
Objective 4 calls this exact script once per lineage stratum via the
-lineage argument, rather than duplicating this file's logic into a
second, separately-maintained copy. Passing -lineage lineage4 restricts
the analysis population to samples whose main_lineage is EXACTLY that
string (see build_analysis_population) before anything else runs; every
downstream step (usable-node filtering, pairwise testing, BH-FDR
correction, the lineage-concentration diagnostic) then operates on that
restricted population using identical code to the pooled run. This
guarantees the pooled and per-lineage results are produced by the same
tested logic and cannot silently drift apart the way two independently
written copies could.
not change that file's column names without checking Objective 3's script.

*** A DIFFERENT QC EXCLUSION THAN OBJECTIVE 1 - READ BEFORE REUSING EITHER
    SCRIPT AS A TEMPLATE FOR THE OTHER ***
Objective 1 excluded 53 samples (qc_flagged_samples.csv's
recommended_exclude_from_phenotype_ground_truth) from its confirmatory
test, because that test compared genotype against phenotype, and 42 of
those 53 samples specifically have an untrustworthy PHENOTYPE label (their
drtype rests on Uncertain-significance evidence, not a confirmed mutation).
This script tests genotype against genotype - mutation vs mutation, not
mutation vs phenotype - so a sample's phenotype trustworthiness is
irrelevant here. Only the 36 samples flagged high_variant_qc (implausible
total_variants_qc, i.e. likely contaminated/misassembled sequencing) are
excluded, because THAT flag calls the sample's variant CALLS themselves
into question, genotype included. The other 17 of the 53 (normal QC,
merely phenotype-inconsistent) are kept in the analysis population here -
excluding them would have no statistical justification for a genotype-only
test and would needlessly shrink the population. Using the Objective 1
exclusion set here, or vice versa, would be a real methodological error,
not a stylistic difference - re-derive the correct exclusion for whichever
kind of test is being run, do not copy one number across scripts blindly.

CONSEQUENCE OF THIS EXCLUSION, CONFIRMED AGAINST THE REAL DATA: dropping
the 36 high-QC samples leaves the analysis population at 1,822 of 1,858
samples, and reduces rpoB|p.Ser431Thr's carrier count from 1 to 0 (its
only carrier was one of the 36) - this node is therefore dropped entirely
from pairwise testing (239 usable nodes, not 240; it remains untouched in
Objective 1's own mutation_node_table.csv, which is cohort-wide and not
QC-filtered). Several other mutations' carrier counts shift down by 1-3
samples for the same reason - this script recomputes every mutation's
carrier count freshly WITHIN the 1,822-sample analysis population rather
than reusing Objective 1's cohort-wide n_samples column, specifically so
...
A pair of mutation nodes is classified using their drugs sets from
mutation_node_table.csv (semicolon-joined, e.g. "rifampicin;rifapentine").
If the two nodes' drug sets share at least one drug in common, the pair is
"within_class" (e.g. two different isoniazid-associated mutations). If the
two drug sets are completely disjoint, the pair is "cross_class" (e.g. a
rifampicin-associated mutation and an isoniazid-associated mutation). This
directly implements the distinction the project plan calls for: within-
class co-occurrence is the expected, lower-priority category; cross-class
is the analytically interesting one - though see the module-level
docstring's note on the MDR-definition confound (Section 4.3 of the
literature review) before treating any single cross-class hit as
conclusive: because MDR/pre-XDR/XDR-TB is clinically DEFINED as multi-drug
resistant, a general tendency for rifampicin- and isoniazid-associated
mutations to co-occur is partly expected by clinical definition, not solely
by biological interaction. The genuinely interesting signal is which
SPECIFIC mutation pairs stand out beyond that general background rate, not
the mere existence of cross-class significant pairs.

A SECOND CONFOUND, FOUND DIRECTLY IN THIS COHORT'S REAL RESULTS, IS FLAGGED
THE SAME WAY: population structure (Section 6.4 of the literature review).
The single strongest within-class pair in this cohort's real results,
rpoB|p.Asp435Gly + rpoB|p.Leu452Pro (odds ratio ~2720, 191 joint carriers),
was checked directly against sample lineage and found to be carried by
lineage4 samples ONLY - 100% concentration, against a ~64% lineage4
baseline in the core-mutation-carrying population. That is exactly the
signature a shared, clonally-inherited genetic background produces, not
necessarily a real biological interaction between the two mutations. This
script therefore computes, for every pair, what fraction of its joint
carriers fall into a single dominant lineage (dominant_lineage_among_both_
carriers, pct_both_carriers_in_dominant_lineage) as a cheap early-warning
diagnostic - NOT a substitute for Objective 4's proper per-lineage
stratified re-analysis, which is the only way to actually resolve whether
...
WHAT THIS SCRIPT DOES, IN ORDER:
  ...
  2. Builds the QC-filtered analysis population (1,822 samples) and
     recomputes each mutation's carrier set within it, dropping any
     mutation left with zero carriers.
  3. Tests every unique pair of the remaining 239 nodes with Fisher's
     exact test (2x2: both / mut1-only / mut2-only / neither), one
     combined Benjamini-Hochberg correction across all pairs together
     (unlike Objective 1, within- and cross-class pairs are NOT separate
     test families - they are the same kind of test, categorised only for
     reporting purposes after the fact).
  4. Splits the results by within/cross-drug-class for reporting, and
     writes a separate, ready-to-use significant-edges file for Objective
...
def build_analysis_population(core_id, qc, y, lineage=None):
    ...
    if lineage is not None:
        # Objective 4 reuse: restrict to samples whose main_lineage is EXACTLY
        # this value (an exact string match against y_labels.csv's main_lineage
        # column) -- this naturally excludes compound-lineage samples like
        # "lineage2;lineage4" from every single-lineage stratum, since neither
        # "lineage2" nor "lineage4" equals that compound string. Combined with
        # the high-QC exclusion above, this reproduces the same two exclusion
        # categories Phase A's lineage_stratification_flags.csv already
        # identified, without needing to re-derive that logic here.
...
def run_pairwise_tests(usable_mutation_ids, carriers_by_mutation, drug_sets, pop_size, sample_lineage):
    ...
    for mid1, mid2 in pairs:
        ...
        # Early-warning lineage-concentration diagnostic: a strong co-occurrence
        # signal driven almost entirely by one lineage is a candidate population-
        # structure confound (a lineage marker masquerading as co-occurrence),
        # exactly the pattern Objective 4's stratified re-analysis is designed to
        # test properly. This is a cheap flag here, not a substitute for that
        # re-analysis -- it exists so a pair like this is never mistaken for a
        # settled finding before Objective 4 has actually checked it.
        both_carriers = c1 & c2
